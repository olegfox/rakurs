...
You can't use this plugin for the first time in your production site, you have to test it before, WHY? Because i made this plugin to work with the basic WordPress install, if you have a lots of plugin (easy load image/some crazy plugin/ other themes) DNUI plugin can or will delete used images in your WordPress site. 

SOO FOR YOUR OWN SECURITY MAKE ONE BIG BACKUP OF YOUR SITE (use other plugin!!!, this one have one very basic backup system, because is not the main purpose) 

If you see any problem with the plugin in the test site, you have options to make a good configuration, Example: the ignore size option, if you use one theme that use Image-size-x.png you can put the size-x in the ignored size and the plugin will not let you to delete the *-size-x.png images (neither the original image).

Why this plugin can't see some used image, because the plugin search in the Database, not in the end generated html code, for resolve this problem the plugin have to make a call to every URL of your site, execute every Javascript in it and see in the HTML generated code if any image is used or not (NOT EASY)

I'm working in the version 2.0 (better code, cleaner and extensible for compatibility) and maybe one paid option to have one server crawler of your site and find every image used and give you one better result.

End of update.


This plugin will help you to find all image in the database that are not being used/unused and give you the opportunity to cleanup the database and the server space.

You will only delete not used/unused image (based in the logic of a blog site, continue to read please).
...
If you have problem with other plugin, you can ask me for help to adapt the code.
