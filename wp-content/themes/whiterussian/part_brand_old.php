...
if ($get == '')
{
    ...
    $sql = "SELECT tax.term_id,tax.parent  FROM $wpdb->term_taxonomy as tax
        ...
        WHERE tax.taxonomy = 'type' AND p.post_type IN ('omron','schneider-electric','siemens','yaskawa','cognex') AND tax.count > 0";
}
/*$sql = "SELECT rl.term_taxonomy_id FROM $wpdb->term_relationships as rl
        LEFT JOIN $wpdb->posts as p 
        ON rl.object_id = p.ID 
        RIGHT JOIN $wpdb->term_taxonomy as tax
        ON rl.term_taxonomy_id = tax.term_taxonomy_id
        WHERE p.post_type = '$get' AND tax.count > 0 AND tax.parent = 0";*/
        else{
        $sql = "SELECT tax.term_id,tax.parent  FROM $wpdb->term_taxonomy as tax
        LEFT JOIN $wpdb->term_relationships as rl
        ON rl.term_taxonomy_id = tax.term_taxonomy_id
        RIGHT JOIN $wpdb->posts as p
        ON rl.object_id = p.ID
        WHERE tax.taxonomy = 'type' AND p.post_type = '$get' AND tax.count > 0";
        }
...
?>
    ...
    <?php if ($all_item->name != ""){ ?>
			<li class="<?php if ($cat_name_new  == $all_item->name){ echo 'active';} ?> b-side-menu__item main-menu-item  menu-item-even menu-item-depth-0 menu-item menu-item-type-taxonomy menu-item-object-brands">
				        <a href="<?php echo bloginfo('url') ?>/?taxonomy=type&type=<?php echo $categor->slug; if ($_GET['post_type']){ echo "&post_type="; echo $get; }  ?>" class="b-side-menu__link menu-item menu-item-type-taxonomy menu-item-object-brands">
