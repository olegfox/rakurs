...
class WebmasterYandex
{
    ...
    public function adminOptions() {
		...
		add_action( 'post_submitbox_misc_actions', array($this, 'metaboxSendTextsToYandexCallback') );
    }
...
//            }
    ...
    public function sendTextToYandex($text) {
		...
		$dom = new DOMDocument();
		if ( $dom->loadXML($response['result']) ) {
			$yandexError = ( $item = $dom->getElementsByTagName('message')->item(0) ) ? $item->nodeValue : 'unknown';
			$yandexId = ( $item = $dom->getElementsByTagName('id')->item(0) ) ? $item->nodeValue : 'unknown';
			$yandexLink = ( $item = $dom->getElementsByTagName('link')->item(0) ) ? $item->getAttribute('href') : 'unknown';
		}
